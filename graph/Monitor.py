...
class Monitor:
    ...
    def save(self):
        ...
        self.weights_[self.idx_] = np.array([neuron.weight_ for neuron in self.lattice_.neurons_.values()])
        self.means_[self.idx_] = np.array([neuron.mean_ for neuron in self.lattice_.neurons_.values()])
        self.covs_[self.idx_] = np.array([neuron.cov_ for neuron in self.lattice_.neurons_.values()])
        self.idx_ += 1
