...
class Graph:
    def __init__(self, n, m, graph_type, adj_list=None):
        ...
        if graph_type == 'rectangular':
            self._create_rectangular(n, m)
        elif adj_list:
            self.adj_list_ = adj_list
            self.nodes_amount_ = len(adj_list)

    def _create_rectangular(self, n, m):
        self.adj_list_ = {}
...
class RectangularLattice(Lattice):
    def generate(self):
        ...
        self.graph_ = Graph(self.size_[0], self.size_[1], 'rectangular')
...
def pcws_reg(x, y, verbose=False):
    ...
    if verbose:
        ...
        plt.xlabel("Number of merged points")
        plt.ylabel('Entropy')
...
def compute_activations(X, units, units_nb):
    activation_vals = np.zeros((X.shape[0], units_nb))

    for i, unit in enumerate(units.values()):
        activation_vals[:, i] = multivariate_normal.pdf(X, unit.mean_, unit.cov_)
...
def compute_responsibilities(X, units, units_nb):
    # For numerical issues
    nu = 744

    units_indexes = list(units.keys())
    responsibilities = np.log(compute_activations(X, units, units_nb))
    ...
    for k in range(units_nb):
        responsibilities[:, k] *= units[units_indexes[k]].weight_
