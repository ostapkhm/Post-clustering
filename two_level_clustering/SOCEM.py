...
class SOCEM:
    def __init__(self, lattice: Lattice, sigma_start, sigma_step, betta, cov_type, tol=1e-4, max_iter=100,
                 random_state=None, reg_covar=1e-6, verbose=False):
    ...
    def estimate_params(self, X: np.ndarray, monitor=None):
        ...
        while sigma > 0:
            ...
            current_ll = self.calculate_ll(X)
            iter_nb = 0
    ...
    def neuron_log_activation(self, X):
        # Return log of neuron activation function
    ...
    def update_nodes(self, X, clusters):
        # Updating lattice neurons based on batch mode
    ...
    def delete_vertex_bhattacharyya(self):
        ...
        vertices = self.lattice_.graph_.adj_list_

        for vertex1 in vertices:
            for vertex2 in vertices[vertex1]:
